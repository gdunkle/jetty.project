...
public class MongoSessionIdManager extends AbstractSessionIdManager {
    ...
    public MongoSessionIdManager(Server server, DBCollection sessions) {
        ...
        _server = server;
        _sessions = sessions;
        _sessions.createIndex(BasicDBObjectBuilder.start().add("id", 1).get(), BasicDBObjectBuilder.start().add("unique", true).add("sparse", false).get());
        _sessions.createIndex(BasicDBObjectBuilder.start().add("id", 1).add("version", 1).get(),
    ...
    protected void scavenge() {
        ...
        /*
         * run a query returning results that: - are in the known list of sessionIds - the expiry time has passed we
         * limit the query to return just the __ID so we are not sucking back full sessions break scavenge query into
         * blocks for faster mongo queries
         */
